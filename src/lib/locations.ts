...
const postsDirectory = path.join(process.cwd(), '/src/posts/achats');
...
export const getSortedDocsData = () => {
  ...
  const allPostsData = fileNames.map((fileName) => {
    ...
    const pageMeta = matter(fileContents).data as ILocation;
...
export const getDocData = async (id: string) => {
  ...
  const pageMeta = matterResult.data as ILocation;
